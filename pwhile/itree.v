...
Section ParSem.
  ...
  Fixpoint com_sem (c : cmd) : cmem -> itree (Call +' E) cmem :=
    ...
    | gassign _ x e => fun m => Ret m.{x <- (esem e m)}
    | x <$- e => fun m =>
                   bind (trigger (GetRnd (esem e m)))
                     (fun t => Ret m.[x <- t])
    | block bs c rs => fun m =>
                   bind (com_sem c (minit m bs))
                     (fun m' => Ret (mret m m' rs))
...
Section SsemLim.
  ...
  Proof.
  elim: c m => [||T x e|T x e|T x e|bs c ih rs
               |e c1 ih1 c2 ih2|e c ih|c1 ih1 c2 ih2|f] m /=.
  - exact: dlim_constE.
  - exact: dlim_constE.
  - exact: dlim_constE.
  - exact: dlim_constE.
  ...
  - transitivity (\dlet_(m' <- dlim (fun n => ssem_aux (l n) c (minit m bs)))
      ...
      by rewrite ih; apply: eq_in_dlet => // m' _; rewrite dlimC.
...
Section CallSem.
  ...
  - rewrite (icm_block bs c rs m) dinterp_bind ih.
    by apply/eq_in_dlet => // m' _; rewrite dinterp_ret.
  - by case: (esem e m); [apply: ih1 | apply: ih2].
  ...
  elim=> [||T y a|T y a|T y a|bs c ih rs
         |a c1 ih1 c2 ih2|a c ih|c1 ih1 c2 ih2|f] m x.
  ...
  - rewrite (dinterp'_eq_itree _ (icm_block bs c rs m)) ssem_aux_blockE.
    apply/(le_trans (le_dinterp'_bind _ _ _ _)).
    by apply: le_dlet; [exact: ih | move=> m' _].
