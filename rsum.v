...
Lemma cvg_dweight_dlim {R : realType} {T : choiceType}
    (mu : nat -> {distr T / R}) :
...
have hfe : ((fun n => \esum_(x in [set: T]) (((predT x)%:R * mu n x)%:E)) @ \oo)%classic
         = ((fun n => (dweight (mu n))%:E) @ \oo)%classic.
...
Lemma le_dfst {R : realType} {C1 C2 : choiceType}
  (nu : {distr (C1 * C2)%type / R}) (p : C1 * C2) :
  nu p <= dfst nu p.1.
Proof.
...
Lemma le_dsnd {R : realType} {C1 C2 : choiceType}
  (nu : {distr (C1 * C2)%type / R} ) (p : C1 * C2) :
  nu p <= dsnd nu p.2.
Proof.
rewrite dmarginE dletE_rsum.
...
(* on a countable set ([misc.esummable_countable_supp]); the union over  *)
(* [n] of those supports is still countable ([bigcup_countable]), and     *)
(* off it every [mu n] is identically [0], so the sequence converges      *)
(* there for free.                                                      *)
(*                                                                      *)
(* [misc.countable_enum] turns that countability into a decoding         *)
(* [nat -> option T] as *data*, which is what lets [diag_cvg] be applied *)
(* at [nat] -- no [countType] instance on a proof-local subtype needs to *)
(* be declared.                                                         *)
(* ==================================================================== *)
Section DCompact.
Context {R : realType} {T : choiceType}.
...
Proof.
pose S := (\bigcup_(n : nat) [set x | mu n x != 0])%classic.
have cS : countable S.
+ apply: bigcup_countable => // n _.
  exact/esummable_countable_supp/summable_mu.
have [g hg] := countable_enum cS.
pose v (n k : nat) : R := if g k is Some s then mu n s else 0.
have bv : forall n k, `|v n k| <= 1.
+ move=> n k; rewrite /v; case: (g k) => [s|]; last first.
  - by rewrite normr0.
  by rewrite ger0_norm ?ge0_mu // le1_mu1.
case: (diag_cvg (u := v) (M := 1) bv) => om homo_om cvg_v.
exists om => // x.
case: (pselect (S x)) => [/hg[k gk]|xNS]; last first.
+ (* [x] is outside every support: the sequence is constantly [0].       *)
  have hz : forall n, mu (om n) x = 0.
  - move=> n; apply/eqP/negPn/negP => hne; apply: xNS.
    by exists (om n).
  apply: (@cvgn_eq _ _ (fun=> 0)); first by move=> n; rewrite hz.
  exact: is_cvg_cst.
apply: (@cvgn_eq _ _ (fun n => v (om n) k)).
+ by move=> n; rewrite /v gk.
exact: cvg_v.
Qed.
...
Proof.
have cS := esummable_countable_supp (summable_mu mu).
have [g hg] := countable_enum cS.
exists (fun i => seq_fset tt (pmap g (iota 0 i))).
move=> x; rewrite in_dinsupp => /hg[k gk].
exists k.+1 => n hn; rewrite seq_fsetE mem_pmap; apply/mapP.
by exists k; [rewrite mem_iota leq0n add0n | rewrite gk].
Qed.
