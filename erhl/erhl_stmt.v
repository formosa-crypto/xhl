...
(*                                                                       *)
(* (2) Countability -- no longer an obstacle.  [strcvg] / [strcvg2]       *)
(*     (strassen.v) require a [countType], and [cmem] is not one and      *)
(*     cannot be: [coremem] is [forall T : IhbType.type, ident -> T]      *)
(*     (pwhile.v), a dependent function type carrying only               *)
(*     [gen_choiceMixin].  But "summable => countable support" *is*       *)
(*     available, in mathcomp/experimental_reals/realsum.v, and           *)
(*     [rsum.dcompact] uses exactly it to lift the Cantor diagonal to an  *)
(*     arbitrary [choiceType].                                           *)
(*                                                                       *)
(* (3) Passing the marginals to the limit.  Lower semicontinuity of the   *)
(*     objective itself is no longer an obstacle -- that is               *)
(*     [misc.espe_fatou], which drops the [nd f] hypothesis that gates    *)
(*     [esum_dlim_r] (analysis/probability_theory/counting_distr.v) and   *)
(*     which a minimising sequence does not satisfy.  But Fatou only ever *)
(*     yields [<=], and the *marginals* must be recovered exactly:        *)
(*     [prhl.iscoupling_dlim] is restricted to monotone sequences         *)
(*     ([nu n <=1 nu m]), so a merely convergent limit needs dominated    *)
(*     convergence; cf. the [DCT_swap] argument in strassen.v.  (Compare  *)
(*     [scoupling_lim] below, which escapes this by pinching the two      *)
(*     marginal inequalities with [dlim_weight1] + [le_rsum_eqP].)        *)
(*                                                                       *)
