...
From xhl.erhl           Require Import erhl_mem erhl_stmt star.
...
Context {R : realType} {A B : codeType} {X Xg Y : eqType}
        {M : erhlMemType A B X Xg}.

Local Notation cmd     := (@cmd_ R A B X Xg M Y).
Local Notation rcond   := (@rcond R A B X Xg M).
Local Notation rcond2  := (@rcond2 R A B X Xg M).
Local Notation rphi    := (@rphi R A B X Xg Y M).
...
Local Notation bexpr   := (@expr_ A B X Xg M bool).
...
Definition ridentg := (@ridentg Xg).

Local Notation rlift := (@ehl_stmt.lift R A B rident ridentg rmem).
...
| H_GAsgn : forall {T1 T2 : B}
    (x : gvars T1) (e1 : expr T1) (y : gvars T2) (e2 : expr T2) f cl ps,
    derivable ps cl
      (fun m : rmem => f (m.{x#g'1 <- `[{e1}] m.1}).{y#g'2 <- `[{e2}] m.2})
      (G x <<- e1) (G y <<- e2) f
...
| H_GAsgnL : forall {T : B} (x : gvars T) (e : expr T) f cl ps,
    derivable ps cl (fun m : rmem => f m.{x#g'1 <- `[{e}] m.1})
      (G x <<- e) skip f
...
Lemma erhl_gassign {T1 T2 : B}
    (x : gvars T1) (e1 : expr T1) (y : gvars T2) (e2 : expr T2) f :
  erhl (fun m : rmem => f (m.{x#g'1 <- `[{e1}] m.1}).{y#g'2 <- `[{e2}] m.2})
       (G x <<- e1) (G y <<- e2) f.
Proof.
move=> m.
set m' := ((m.{x#g'1 <- `[{e1}] m.1}).{y#g'2 <- `[{e2}] m.2} : rmem).
...
Lemma erhl_gassignL {T : B} (x : gvars T) (e : expr T) f :
  erhl (fun m : rmem => f m.{x#g'1 <- `[{e}] m.1}) (G x <<- e) skip f.
Proof.
move=> m; set m' := (m.{x#g'1 <- `[{e}] m.1} : rmem).
...
Lemma erhl_gassignR {T : B} (x : gvars T) (e : expr T) f :
  erhl (fun m : rmem => f m.{x#g'2 <- `[{e}] m.2}) skip (G x <<- e) f.
Proof.
move=> m; set m' := (m.{x#g'2 <- `[{e}] m.2} : rmem).
have h1 : ssem_ ps skip m.1 = dunit (m' # '1)%M.
+ by rewrite ssemE /m' mselect_msetg.
